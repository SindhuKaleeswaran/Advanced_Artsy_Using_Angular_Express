...
app.post('/login', async(req,res) => {
    try {
        ...
        res.cookie('jwtoken', jwtoken, { 
            ...
            domain: 'localhost', // or your domain if needed
...
/*app.get('/mee', async (req, res) => {
    ...
    try {
        ...
        console.log('Decoded token:', decoded);
        const user = await user.collection.findById(decoded.userId);
        ...
        res.status(200).json({ fullname: user.fullname, email: user.email, profileImageUrl: user.profileImageUrl });
    } catch (error) {
...
app.post('/fav',authenticateToken, async (req, res) => {
    try {
        ...
        if(flag === 1){ //that is add artist id to array
            ...
            await existingUser.save();
            return res.status(200).json({ message: 'Artist deleted to favourites'});
        }
